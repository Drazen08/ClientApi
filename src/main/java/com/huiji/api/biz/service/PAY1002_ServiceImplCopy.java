...
public class PAY1002_ServiceImplCopy {
    ...
    public void executeService(IResponse response, IRequest<PAY1002_ReqBody> request,String uid)throws Exception{
...
//                    if (orderDB.getPid() == null) {//说明是主订单
                    flag=yueZhiFu(orderDB, orderDB.getPrice(), uid, strDay, strMonth, strYear,1,RunningNum);
//                    }else{
//                        flag=yueZhiFu(orderDB, orderDB.getPrice(), uid, strDay, strMonth, strYear,2);
//                    }
                    if(flag)
                        sb.append("订单号："+orderId+"支付成功");
                    else
                        sb.append("订单号："+orderId+"支付失败");
                }else{//余额不够支付订单的
//                    Long wxZF=Math.abs(balance);
//                    Long yueZH=orderDB.getPrice()+balance;//15-5=10
//                    balance=orderDB.getPrice()+balance-yueZH;
...
//                        if (oderLastPay != null) {
//                            if (oderLastPay.getState() != 4000) {
//                                sb.append("订单号：" + orderId + "不能再次支付;");
//                                continue;
//                            }
//                        }
...
//                        if (orderPayTaskDB.getState() == 2000 || oderLastPay.getState() != 4000) {
//                            sb.append("订单号：" + orderId + "不能再次支付;");
//                            continue;
//                        }
                        payCode= "WXZF"+OrderCodeUtil.getOrderNo();//生成支付单号
...
//                        orderPayTaskMapper.updateWeiXinCount(orderId, ++count);
                    ...
                    if(odkUserBillDetail==null&&yueZH!=0){
                        yueZhiFu(orderDB, yueZH, uid, strDay, strMonth, strYear, 2,RunningNum);
                    }
                    if(odkUserBillDetail!=null){
                        String payCodeYE= "YEZF" + OrderCodeUtil.getOrderNo();//生成支付单号
//                        UserBillDetail userBillDetail = new UserBillDetail(RunningNum,uid, payCodeYE, orderId,orderDB.getPrice(), 0L, yueZH,odkUserBillDetail.getBlance1000(),odkUserBillDetail.getBlance2000(), 3, 10, createDate,
...
//                    UserBillDetail userBillDetail = new UserBillDetail(RunningNum,uid, payCode, orderId,orderDB.getPrice(), wxZF, 0L,0L,0L, 2, 0, createDate,
//                            strDay, strMonth, strYear);
//                    userBillDetailMapper.saveUserBillDetail(userBillDetail);
    ...
    private Boolean yueZhiFu(Order order, Long balance, String uid, String str_day, String str_month, String str_year, int type,String RunningNum) {
...
//                Map<String, Object> map = new HashMap<String, Object>();
//                map.put("type", 10);
//                map.put("send_type", 10);
//                map.put("pay_type", 10);
//                map.put("type_desc", "已发货，未确认收货");
//                map.put("endprice", 0);
//                map.put("balance", balance);
//                map.put("paytime", DateUtil.dateToString(new Date()));
//                map.put("payid", 1);
...
//                map.put("order_id", orderId);
//                orderMapper.updateOrderStauts2(map);
//
//            }
...
//            UserBillDetail userBillDetail = new UserBillDetail(RunningNum,uid, payCode, orderId,order.getPrice(), 0L, balance,useBalance1000,useBalance2000, 3, 10, createDate,
